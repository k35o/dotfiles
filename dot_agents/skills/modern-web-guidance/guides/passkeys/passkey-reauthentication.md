...
**アカウント所有権を検証する**: 検証エンドポイントは、クライアントから返された認証成功時のクレデンシャルIDが、保存されているクレデンシャルレコードに解決でき、かつそのユーザーIDがサインイン中のアクティブユーザーと厳密に一致する（`storedCredential.passkeyUserId === req.user.id`）ことを必ず明示的に検証してください。_別_ユーザーの有効なパスキーが返された場合、認証は即座に拒否しなければなりません。
